...
TutorialLevel::TutorialLevel(sf::RenderWindow* hwnd, Input* in, GameState* gs, AudioManager* aud, TextureManager* tm)
{
	window = hwnd;
	input = in;
	gameState = gs;
...
// handle user input
void TutorialLevel::handleInput(float dt)
{
	if (damagedTimer > 0)
	{
		damagedTimer -= dt;
...
*/
void TutorialLevel::resetPlayer()
{
